...
def open_ome_zarr(spec, *, storage_options=None):
    """Inspect an OME-Zarr input; reject ambiguous or uncalibrated geometry.

    Fragment ``spacing=x,y,z`` overrides spacing *at the selected level* in
    mm; ``origin=x,y,z`` overrides its world origin in mm; ``direction`` is
    nine row-major orthonormal direction cosines. ``unit`` supplies missing
    spatial units, but cannot replace conflicting declared units. Geometry
    overrides warn and never suppress unsupported-transform errors.
    ``storage_options`` forwards remote configuration to fsspec.
    """
    try:
        import zarr
    ...
    try:
        if int(zarr.__version__.split(".")[0]) >= 3:
            root = zarr.open_group(store, mode="r", use_consolidated=False,
                                   **({"storage_options": access} if access else {}))
        else:
            if "://" in location:
                import fsspec
    ...
    if version is None and "ome" not in attributes:
        ...
        if (isinstance(candidate_axes, list) and candidate_axes
                and all(isinstance(a, dict) for a in candidate_axes)
                and isinstance(candidate_datasets, list) and candidate_datasets
                and all(isinstance(d, dict) and "coordinateTransformations" in d for d in candidate_datasets)):
            version = "0.4"
    ...
    for name, size in zip(names, shape):
        if name in "xyz":
            ...
            if selector == "channel" and not value.lstrip("-").isdigit():
                omero = metadata.get("omero", {})
                channels = omero.get("channels", []) if isinstance(omero, dict) else []
                matches = [i for i, c in enumerate(channels)
                           if isinstance(c, dict) and c.get("label") == value]
                if len(matches) != 1:
    ...
    scale, offset = scale * group_scale, offset * group_scale + group_offset
    ...
    for key in ("spacing", "origin", "direction", "unit"):
        if key in options:
            warnings.warn(f"OME-Zarr {path}: explicit {key}={options[key]}; calibration applies to selected level {level}.", UserWarning, stacklevel=2)
    spatial_names = [n for n in names if n in "xyz"]
    return OMEZarrVolume(array, tuple(selection), tuple(spatial_names.index(n) for n in "zyx"),
                         shape_xyz, spacing, origin, direction, path, version)
